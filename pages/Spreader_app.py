...
from api_clients.api_client_spreader import post_to_sap_spreader  # 🔗 Integration with Flask + MongoDB

# ==============================
# 🧾 PAGE CONFIGURATION
# ==============================
st.set_page_config(page_title="Spreader Roll Sliver Weight Khata", layout="centered")
st.title("🧾 Spreader Roll Sliver Weight Khata")
...
quality_types = ["B", "(B)", "K", "J", "S", "T", "A", "P", "O", "I I", "H", "M"]
...
flagged_error = False  # 🚩 Flag for invalid entries
...
avg_converted_weight = 0.0
if converted_weight1 > 0 or converted_weight2 > 0:
    avg_converted_weight = (converted_weight1 + converted_weight2) / 2

st.success(f"✅ Average Converted Weight: {avg_converted_weight:.4f} kg")
...
if st.button("📤 Submit Record"):
    if flagged_error:
        st.error("🚩 Submission blocked due to invalid entries. Please correct the highlighted fields.")
    else:
        record = {
            ...
            "Average Converted Weight (kg)": round(avg_converted_weight, 4),
            "Remarks": remarks,
        ...
        try:
            existing = pd.read_csv("spreader_records.csv")
            updated = pd.concat([existing, df], ignore_index=True)
            updated.to_csv("spreader_records.csv", index=False)
        except FileNotFoundError:
            df.to_csv("spreader_records.csv", index=False)

        # --- Push to Flask + MongoDB ---
        status = post_to_sap_spreader(record)
        if status == "success":
...
st.markdown("---")
st.caption("Developed for Spreader Department — Streamlit + Flask + MongoDB Integration")
